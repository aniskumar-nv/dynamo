...
def handle_transport_info(daemon: "Daemon", msg: Message) -> Response:
...
def handle_transport_add_peer(daemon: "Daemon", msg: Message) -> Response:
...
def handle_transfer_blocks_batch(daemon: "Daemon", msg: Message) -> Response:
...
def handle_read_bootstrap_into_staging(daemon: "Daemon", msg: Message) -> Response:
    return daemon._read_bootstrap_into_staging(msg)


def handle_fetch_remote(daemon: "Daemon", msg: Message) -> Response:
...
def handle_register_bootstrap_handle(daemon: "Daemon", msg: Message) -> Response:
...
def handle_get_bootstrap_info(daemon: "Daemon", msg: Message) -> Response:
                ...
                try:
                    with lease as slot:
                        daemon.transport.register_buffer(
                            slot.host_ptr,
                            int(size),
                            label=(f"host:{content_hash.hex()[:8]}:" f"{int(layer)}"),
                        )
                        regions.append(
                            {
                                "ptr": slot.host_ptr,
